...
This will spin up the flask server which responds to the request coming through the '<port>'. We are still needed to initialize the DB separately, which is being covered in the following section. 
...
This will prompt the message "Initialized the database" and will instantiate a SQLite DB. Everytime flask app is launched either by using gunicorn(linux/MacOS) or waitress(Windows), the all the tables will be dropped and the same tables will be instantiated again. Empty this time.
